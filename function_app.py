...
def handle_pit_data(data, container_name):
    ...
    container_client = blob_service_client.get_container_client(container=container_name) 
    with open(file="/tmp/existing.csv", mode="wb") as download_file:
        download_file.write(container_client.download_blob(f"{container_name}_pit.csv").readall())
    existing_df = pd.read_csv("/tmp/existing.csv")
    # Drop any existing data with the same key
    ...
    logging.info('Saving to blob storage.')
    # Create a blob client using the local file name as the name for the blob
    blob_client = blob_service_client.get_blob_client(container=container_name, blob=f"{container_name}_pit.csv")
    with open(file=local_file_name, mode="rb") as blob_data:
        blob_client.upload_blob(blob_data, overwrite=True)
        
    # Save the raw JSON to blob storage
    blob_client = blob_service_client.get_blob_client(container="raw", blob="pit_" + pit_data["key"]+".json")
    with open(file=raw_path, mode="rb") as blob_data:
        blob_client.upload_blob(blob_data, overwrite=True)
